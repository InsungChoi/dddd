		...
		public bool FileLoad(string strFileName)
		{
			...
			if(File.Exists(strFileName) == true)
			{
				...
				else if(System.IO.Path.GetExtension(strFileName) == ".xls")
				{
					...
					try
					{
						xlBook = xlApp.Workbooks.Open(strFileName, 0, false, 5, "", "", true, Excel.XlPlatform.xlWindows, "\t", true, true, 0, true,false,false);
						Excel.Sheets sheets = xlBook.Worksheets;
						xlSheet = (Excel.Worksheet)sheets.get_Item(1);

						while(((Excel.Range)xlSheet.Cells[count,"A"]).Text.ToString() != "")
						{
							// strWord.Length == 2
							if(((Excel.Range)xlSheet.Cells[count,"C"]).Text.ToString() == "")
							{
								Frequency.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"A"]).Text.ToString()));
								MAbsorption.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"B"]).Text.ToString()));

								result = true;
							}
							else if(((Excel.Range)xlSheet.Cells[count,"F"]).Text.ToString() != "") // strWord.Length == 6
							{
								Frequency.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"A"]).Text.ToString()));
								MAbsorption.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"B"]).Text.ToString()));
					
								//double Temp_4Col = double.Parse(((Excel.Range)xlSheet.Cells[count,"D"]).Text.ToString());
								//double Temp_5Col = double.Parse(((Excel.Range)xlSheet.Cells[count,"E"]).Text.ToString());
					
								//MSurfaceImpedance.AddData(double.Parse(Math.Sqrt(Math.Pow(Temp_4Col,2) + Math.Pow(Temp_5Col,2)).ToString()));
								MRealSurfaceImpedance.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"E"]).Text.ToString()));
								MImagSurfaceImpedance.AddData(double.Parse(((Excel.Range)xlSheet.Cells[count,"F"]).Text.ToString()));
