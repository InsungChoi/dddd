...
Zmt=j*(omega*ms-D*kx^4/omega);	% mechanical impedance of panel for transverse motion
Zmp=j*(omega*ms-Dp*kx^2/omega);	% mechanical impedance of panel for longitudinal motion
...
HM(2,1)=j*omega*GM(2,1);
HM(2,2)=j*omega*GM(2,2);
HM(2,3)=-j*omega*((1-h)*GM(2,3)+h*GM(2,4));
...
deno=-(HM(1,2)-HM(5,2)/j/omega/Zmp-kx*hp*HM(2,2)/2/omega);

S1=(HM(1,1)-HM(5,1)/j/omega/Zmp-kx*hp*HM(2,1)/2/omega)/deno;
S2=(HM(1,3)-HM(5,3)/j/omega/Zmp-kx*hp*HM(2,3)/2/omega)/deno;
S3=Zmt*HM(2,1)-HM(3,1)-HM(4,1)+j*kx*hp*HM(5,1)/2;
S4=Zmt*HM(2,2)-HM(3,2)-HM(4,2)+j*kx*hp*HM(5,2)/2;
S5=Zmt*HM(2,3)-HM(3,3)-HM(4,3)+j*kx*hp*HM(5,3)/2;

TM(1,1)=S2*S4+S5-h*S2*(S1*S4+S3)/(1-h+h*S1);
TM(1,2)=(S1*S4+S3)/j/omega/(1-h+h*S1);
TM(2,1)=HM(2,2)*S2+HM(2,3)-h*S2*(HM(2,2)*S1+HM(2,1))/(1-h+h*S1);
TM(2,2)=(HM(2,2)*S1+HM(2,1))/j/omega/(1-h+h*S1);
